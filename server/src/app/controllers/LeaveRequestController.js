...
const LeaveRequestController = {
  ...
  createLeaveRequest: async (req, res) => {
    try {
      const { student_id, parent_id, teacher_id, class_id, start_date, end_date, reason, sessions } = req.body;
      ...
      const existingDates = existingLeaveRequests.map((req) => new Date(req.start_date).toISOString().split("T")[0]);

      // Lọc ra các ngày chưa có đơn
      const availableDates = dateRange.filter((date) => !existingDates.includes(date.toISOString().split("T")[0]));
      ...
      console.log("Đơn xin nghỉ học đã được tạo thành công:", savedLeaveRequest._id);
    } catch (error) {
      console.error("Lỗi khi tạo đơn xin nghỉ học:", error);
      res.status(500).json({
        success: false,
  ...
  getLeaveRequestsByStudentId: async (req, res) => {
    try {
      ...
      const student = await Student.findById(student_id).select("userName").lean();
  ...
  getLeaveRequestsByParentId: async (req, res) => {
    try {
      ...
      const studentIds = [...new Set(leaveRequests.map((request) => request.student_id))];

      // Lấy thông tin tên của tất cả học sinh liên quan
      const students = await Student.find({ _id: { $in: studentIds } }, { userName: 1 }).lean();
  ...
  updateLeaveRequest: async (req, res) => {
    try {
      ...
      const leaveRequest = await LeaveRequest.findOneAndUpdate({ _id: leaveRequest_id }, { status }, { new: true });
