...
const LeaveRequestController = {
  ...
  createLeaveRequest: async (req, res) => {
    try {
      ...
      // Kiểm tra các ngày đã có đơn xin nghỉ với điều kiện chính xác hơn
      const existingLeaveRequests = await LeaveRequest.find({
        student_id,
        status: { $in: ['pending', 'approved'] },
        $or: [
          // Kiểm tra xem request mới có chồng lấn với các request đã tồn tại không
          {
      ...
      // Nếu có bất kỳ request nào trùng
      if (existingLeaveRequests.length > 0) {
        return res.status(400).json({
          success: false,
          message: 'Đã tồn tại đơn xin nghỉ cho một hoặc nhiều ngày được chọn',
          existingRequests: existingLeaveRequests,
        })
      }
      ...
      // Tạo sessions mới cho các ngày còn lại
      const newSessions = dateRange.map((date) => ({
        date: date,
        morning: sessions[0]?.morning || false,
        afternoon: sessions[0]?.afternoon || false,
      }))
      ...
      res.status(201).json({
        success: true,
        message: `Đơn xin nghỉ học đã được tạo thành công cho ${dateRange.length} ngày`,
        data: savedLeaveRequest,
      })
    } catch (error) {
      console.error('Lỗi khi tạo đơn xin nghỉ học:', error)
