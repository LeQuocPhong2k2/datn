...
const NotificationController = {
  ...
  editNotification2: async (req, res) => {
    try {
      const { _id, subject, text, link, image, notification_time } = req.body
      const notification = await Notification.findById(_id)
      if (!notification) {
        return res.status(404).json({
          success: false,
      ...
      notification.content.subject = subject
      notification.content.text = text
      notification.content.link = link
      notification.content.image = image
