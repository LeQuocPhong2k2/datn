...
const ClassController = {
  ...
  getAllClassTeacher: async (req, res) => {
    ...
    try {
      ...
      const listTeacherSchedule = await Schedule.find({
      ...
      for (let i = 0; i < listTeacherSchedule.length; i++) {
        const classInfo = await Class.findOne({
          academicYear: currentSchoolYear,
          className: listTeacherSchedule[i].className,
        });
        if (classInfo) {
  ...
  // hàm lấy id của giáo viên chủ nhiệm của lớp học từ className, academicYear
  getHomeRoomTeacherByClassNameAndAcademicYear: async (req, res) => {
    const { className, academicYear } = req.body;
    try {
      const classInfo = await Class.findOne({
        className: className,
      ...
      // lấy ra homeRoomTeacher từ classInfo
  ...
  checkHomeRoomTeacher: async (req, res) => {
    const { phoneNumber, className } = req.body;
    try {
      const teacher = await Teacher.findOne({ phoneNumber: phoneNumber });
      if (!teacher) {
        return res.status(404).json({ message: "Không tìm thấy giáo viên" });
      ...
      if (!classInfo) {
        return res.status(404).json({ message: "Giáo viên không phải chủ nhiệm lớp" });
      }

      res.status(200).json({ message: "Giáo viên chủ nhiệm lớp" });
    } catch (error) {
      console.error("Lỗi khi kiểm tra giáo viên chủ nhiệm:", error);
      res.status(500).json({ error: error.message });
    }
  },
};
