...
export default function QuanLyHocSinh({ functionType }) {
  ...
  const resetSearch = () => {
    ...
    setShowMenuProfileStudent(null);
  ...
  // sự kiện khi mà tìm kiếm nhấp vào tên học sinh sẽ xổ ra menu trạng thái Xem hồ sơ , Sửa hô sơ , Xóa hồ sơ , Chuyển lớp
  const [showMenuProfileStudent, setShowMenuProfileStudent] = useState(null);
  const handleShowMenuProfileStudent = (id) => {
    setShowMenuProfileStudent(showMenuProfileStudent === id ? null : id); // Nếu ID đã mở, đóng menu, ngược lại mở menu
  };

  // sự kiện componet khi bấm vào ví dụ sửa hồ sơ
  const [isShowComponetStudent, setShowComponetStudent] = useState({
    studentInfo: false,
    studentUpdate: false,
    studentDelete: false,
  });
  // lưu 1 biến studentInfoUpdate để lưu thông tin học sinh cần sửa
  // Component cha
  const [studentInfoUpdate, setStudentInfoUpdate] = useState({});
  const handleEditProfileStudent = async (studentCode) => {
    const res = await getFullInfoStudentByCode(studentCode);
    console.log(res);
    setStudentInfoUpdate(res);
    setShowMenuProfileStudent(null);
    setShowComponetStudent({
      studentInfo: false,
      studentUpdate: true,
      studentDelete: false,
    });
  };
  const handleDeleteProfileStudent = async (studentCode) => {
    setShowMenuProfileStudent(null);

    // gọi tới deleteStudent với tham số là mã số sinh viên trước khi xoá cần confirm xác nhận xoá học sinh hay không
    if (window.confirm('Bạn có chắc chắn muốn xoá học sinh này không?')) {
      const res = await deleteStudent(studentCode);
      console.log(res);
  ...
  // Render UpdateStudent component conditionally

  return (
    <>
      {isShowComponetStudent.studentUpdate ? (
        <UpdateStudent
          setShowComponetStudent={setShowComponetStudent}
          isShowComponetStudent={isShowComponetStudent}
          studentInfoUpdate={studentInfoUpdate} // Truyền thông tin sinh viên vào UpdateStudent
        />
      ) : (
        <>
                      ...
                      studentbyResearch.map((student, index) => (
                        <tr key={student._id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap relative  ">
                            ...
                            {/* Menu thả xuống */}
                            {showMenuProfileStudent === student._id && ( // Kiểm tra xem menu có đang mở không
          ...
          {functionType === 'add-student-import' && <ImportStudent />}
          {/* </> */}
        </>
      )}
    </>
